...
本目录以**稳定标识符 `id`** 为主键组织条目，并按「组织 / 地点 / **系统**（人工智能·桥）/ **规则**（制度机制）/ 文化 / 人物」落在对应子文件夹里，方便人手翻阅。

**勿混**：`系统/` 只收**桥**类管理者 AI 实例；`规则/` 收人事部办事制度（编组、冲突、优先序、责任切割等）。二者 `id` 均可为 `SYS-*`，以 [id-registry](../../治理/id-registry.yaml) 登记路径为准。
...
| [SYS-WORKFORCE](规则/SYS-WORKFORCE-员工来源与编组风险.md) | 员工来源与编组风险（**制度规则**） |
| [SYS-CONFLICT](规则/SYS-CONFLICT-员工冲突机制.md) | 员工冲突机制（**制度规则**） |
| [SYS-DELEGATION](规则/SYS-DELEGATION-外包签字与责任转嫁.md) | 外包、签字与责任转嫁（**制度规则**） |
